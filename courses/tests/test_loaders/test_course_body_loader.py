...
from ...loaders.CourseBodyLoader import CourseBodyLoader, course_body_loader
...
class CourseBodyLoaderTestCase(TestCase):
    databases = {'master', }

    @classmethod
    def setUpTestData(cls):
        cls.tested_class = CourseBodyLoader
        cls.batch_load_fn = course_body_loader
    ...
    def test_When_UseBatchLoadFN_Should_ReturnListOfQuerySet(self):
        ...
        queryset = CourseBody.objects.filter(**{
            'course_id__in': keys,
        })

        expected_data = [
            *[item.body for item in queryset],
            None,
        ]
